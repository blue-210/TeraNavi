...
create table tera_db.users(user_id int(8) primary key auto_increment,login_id varchar(30) not null unique,
						   ...
						   admin_flag char(1) not null,last_login_date char(100),admin_last_login_date char(100),user_status_flag char(1) not null,user_lock_start_date date,
						   user_lock_end_date date,blog_explanation varchar(100),user_profile varchar(2000),blog_status_flag char(1) default '0',index(user_id),index(user_name),
...
create table tera_db.contacts(contact_id int(10) primary key auto_increment,contact_user_name varchar(50) not null,contact_title varchar(160) not null,
							  contact_body varchar(10000),contact_date datetime not null,
							  contact_address varchar(40) not null,contact_category varchar(100),index(contact_id));
#警告表
create table tera_db.cautions(caution_id int(10) primary key auto_increment,fk_user_id int(10),fk_caution_user_id int(10),caution_date datetime,caution_title varchar(100) not null,
...
#登録キー表
create table tera_db.sign_up_keys(sign_up_key varchar(140) primary key,key_status char(1) not null,index(sign_up_key));
#ポリシー表
...
#利用規約表
create table tera_db.rules(rule_id int(10) primary key auto_increment,rule_date datetime,rule_body varchar(10000) not null,index(rule_id));
